...
accel_x = accel(:,1);  % load dataset 1
accel_y = accel(:,2);  % load dataset 2
accel_z = accel(:,3);  % load dataset 3


% Split each dataset into training and validation sets
% cv = cvpartition(size(accel_x,1),'HoldOut',0.3);
% idx = cv.test;
num = numel(accel_x);
...
XTrain1 = accel_x(idxXTrain)';  
YTrain1 = accel_x(idxYTrain)';  
XVal1 = accel_x(idxXVal)';
YVal1 = accel_x(idxYVal)';

% cv = cvpartition(size(accel_y,1),'HoldOut',0.3);
% idx = cv.test;
% 
% XTrain2 = accel_y(~idx,:);
% XVal2 = accel_y(idx,:);
...
XTrain2 = accel_y(idxXTrain)';
YTrain2 = accel_y(idxYTrain)';
XVal2 = accel_y(idxXVal)';
YVal2 = accel_y(idxYVal)';

% cv = cvpartition(size(accel_z,1),'HoldOut',0.3);
% idx = cv.test;
% 
% XTrain3 = accel_z(~idx,:);
% XVal3 = accel_z(idx,:);
...
XTrain3 = accel_z(idxXTrain)';
YTrain3 = accel_z(idxYTrain)';
XVal3 = accel_z(idxXVal)';
YVal3 = accel_z(idxYVal)';
...
writematrix(time,'AccelPred.xls','Range','A3:A3000');
writematrix(YPred1,'AccelPred.xls','Range','B3:B3000');
writematrix(YPred2,'AccelPred.xls','Range','C3:C3000');
writematrix(YPred3,'AccelPred.xls','Range','D3:D3000');
