...
mag_x = mag(:,1);  % load dataset 1
mag_y = mag(:,2);  % load dataset 2
mag_z = mag(:,3);  % load dataset 3


% Split each dataset into training and validation sets
% cv = cvpartition(size(mag_x,1),'HoldOut',0.3);
% idx = cv.test;
num = numel(mag_x);
...
XTrain1 = mag_x(idxXTrain)';  
YTrain1 = mag_x(idxYTrain)';  
XVal1 = mag_x(idxXVal)';
YVal1 = mag_x(idxYVal)';

% cv = cvpartition(size(mag_y,1),'HoldOut',0.3);
% idx = cv.test;
% 
% XTrain2 = mag_y(~idx,:);
% XVal2 = mag_y(idx,:);
...
XTrain2 = mag_y(idxXTrain)';
YTrain2 = mag_y(idxYTrain)';
XVal2 = mag_y(idxXVal)';
YVal2 = mag_y(idxYVal)';

% cv = cvpartition(size(mag_z,1),'HoldOut',0.3);
% idx = cv.test;
% 
% XTrain3 = mag_z(~idx,:);
% XVal3 = mag_z(idx,:);
...
XTrain3 = mag_z(idxXTrain)';
YTrain3 = mag_z(idxYTrain)';
XVal3 = mag_z(idxXVal)';
YVal3 = mag_z(idxYVal)';
...
writematrix(time,'MagPred.xls','Range','A3:A3000');
writematrix(YPred1,'MagPred.xls','Range','B3:B3000');
writematrix(YPred2,'MagPred.xls','Range','C3:C3000');
writematrix(YPred3,'MagPred.xls','Range','D3:D3000');